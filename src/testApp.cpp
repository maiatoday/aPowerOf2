...
void testApp::setup() {
	if (XML.loadFile("mySettings.xml")) {
		...
		fullscreen =
				(XML.getValue("GENREAL:FULLSCREEN", 1) == 1) ? true : false;
...
void testApp::update() {
	//updateKinect();
	//get the UserCenters from the kinect
...
void testApp::updateKinect() {
	...
	if (isLive) {
		...
		userCenters.clear();
		int numUsers = recordUser.getNumberOfTrackedUsers();
		for (int i = 0; i<numUsers; i++) {
			ofxTrackedUser* user = recordUser.getTrackedUser(i);
			ofPoint point;
...
//	red = 233; blue = 52; green = 27;
	...
	gui1->addWidgetDown(
			new ofxUILabel("Press 'h' to Hide GUIs", OFX_UI_FONT_LARGE));
...
//	gui1->addSlider("BLUE", 0.0, 255.0, blue, length-xInit,dim);
	...
	gui1->addRadio("RADIO HORIZONTAL", names, OFX_UI_ORIENTATION_HORIZONTAL,
			dim, dim);
	gui1->addRadio("RADIO VERTICAL", names, OFX_UI_ORIENTATION_VERTICAL, dim,
			dim);
	...
	gui1->addRangeSlider("RSLIDER", 0.0, 255.0, 50.0, 100.0, length - xInit,
			dim);

	gui1->addSpacer(length - xInit, 2);
	gui1->addWidgetDown(new ofxUILabel("2D PAD", OFX_UI_FONT_MEDIUM));
	gui1->add2DPad("PAD", ofPoint(0, length - xInit), ofPoint(0, 120),
			ofPoint((length - xInit) * .5, 120 * .5), length - xInit, 120);
...
//    for(int i = 0; i < 256; i++) { buffer[i] = ofNoise(i/100.0); }
	...
	gui2 = new ofxUICanvas(length + xInit + 2, 0, length + xInit,
			ofGetHeight());
	...
	gui2->addWidgetDown(
			new ofxUILabel("AUTO CLEAR DISABLED", OFX_UI_FONT_MEDIUM));
	gui2->addTextInput("TEXT INPUT2", "Input Text", length - xInit)->setAutoClear(
			false);
...
//	gui2->addWidgetDown(new ofxUIImage(img->width*.5, img->height*.5, img, "IMAGE CAPTION"));
	...
	gui2->addWidgetDown(
			new ofxUINumberDialer(-10000, 10000, 5000, 1, "NUMBER DIALER",
					OFX_UI_FONT_MEDIUM));
